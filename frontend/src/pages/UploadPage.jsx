...
const ImageUploadStep = React.memo(({ images, mainImage, handleImageUpload, setMainImage }) => {
    ...
    return (
        <div className="space-y-4">
            <div className={`relative border-2 border-dashed rounded-lg p-8 text-center transition-colors ${dragActive ? "border-blue-500 bg-blue-50" : "border-gray-300"}`} onDragEnter={handleDrag} onDragLeave={handleDrag} onDragOver={handleDrag} onDrop={handleDrop}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-2">
                    ...
                    <p>
                        <span className="font-medium">Koordináták:</span> {formData.location[0].toFixed(4)}, {formData.location[1].toFixed(4)}
                    </p>
                </div>
            </div>
        ...
        <div className="w-full min-h-screen bg-gray-50">
            <div className="w-full flex flex-row items-center justify-between bg-white px-8 py-4 shadow-sm">
                ...
                <div className="flex items-center justify-center">
                    {[
                        ...
                        <div key={num} className="flex items-center">
                            <button onClick={() => goToStep(num)} className="flex flex-row items-center gap-3 focus:outline-none" disabled={isSubmitting}>
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${step >= num ? "bg-blue-500 text-white" : "border-2 border-gray-300 text-gray-400"} ${isSubmitting ? "opacity-50 cursor-not-allowed" : ""}`}>{step > num ? <IconCheck size={20} /> : num}</div>
                                <span className={`text-sm font-medium ${step === num ? "text-blue-600" : "text-gray-500"}`}>{title}</span>
                            </button>
                ...
                <div className="flex gap-2">
                    <button onClick={prevStep} disabled={step === 1 || isSubmitting} className="px-4 py-2 text-sm font-medium text-gray-600 border border-gray-300 rounded-md hover:border-gray-400 hover:text-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
                        Vissza
                    </button>
                    <button onClick={step === 4 ? handleSubmit : nextStep} disabled={isSubmitting} className={`px-4 py-2 text-sm font-medium text-white rounded-md transition-colors ${step === 4 ? "bg-green-500 hover:bg-green-600" : "bg-blue-500 hover:bg-blue-600"} disabled:opacity-50 disabled:cursor-not-allowed`}>
