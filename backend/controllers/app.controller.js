...
export const savePost = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Post saved successfully", savePost });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
export const updateAvatar = async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.storage
      .from("avatars")
      .upload(
        `avatars/${req.userId}/${req.file.filename}`,
    ...
    const publicUrl = supabase.storage
      .from("avatars")
      .getPublicUrl(`avatars/${req.userId}/${req.file.filename}`)
      .data.publicUrl;
    ...
    await Promise.all(
      req.files.map(async (file) => {
        if (!allowedMimeTypes.includes(file.mimetype)) {
          throw new Error(
            "Csak JPEG, PNG vagy JPG formátumú képek engedélyezettek"
          );
        }
        ...
        const { error } = await supabase.storage
          .from("listing_images")
          .upload(uploadPath, fs.readFileSync(watermarkedPath), {
